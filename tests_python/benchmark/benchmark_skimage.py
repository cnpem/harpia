...
repetitions = 1
reps = 30
nslices_values = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512]
#more slices takes too long to execute

image_sets = [
    ("binary", images_binary, image_binary, operations.binary_skimage),
    ("grayscale", images_grayscale, image_grayscale, operations.grayscale_skimage)
]

for nslices in nslices_values:
    for image_type, dtypes, image, ops in image_sets:
        csv_file = f"results_mary/{machine}_{reps}reps_skimage{nslices}_{image_type}.csv"
