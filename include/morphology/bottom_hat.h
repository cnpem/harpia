...
/**
 * @brief Computes the bottom-hat transformation of a grayscale image on the GPU.
 *
 * The bottom-hat transformation enhances dark structures in an image by computing the difference 
 * between the morphological closing of the input image and the original image. This implementation 
 * utilizes CUDA for efficient execution on the GPU.
 *
 * The morphological operations (dilation followed by erosion) are performed using a 3D structuring 
...
/**
 * @brief Computes the bottom-hat transformation of a grayscale image on the CPU.
 *
 * The bottom-hat transformation enhances dark structures in an image by computing the difference 
 * between the morphological closing of the input image and the original image. This implementation 
 * is performed entirely on the host CPU.
 *
 * The transformation applies a sequence of morphological operations (dilation followed by erosion) 
