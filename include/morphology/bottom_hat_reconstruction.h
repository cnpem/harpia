#ifndef BOTTOM_HAT_RECONSTRUCTION_H
#define BOTTOM_HAT_RECONSTRUCTION_H

 /**
 * @brief Computes a specialized bottom-hat transformation of a grayscale image on the GPU.
 *
 * The bottom-hat transformation enhances dark structures in an image by computing the difference 
 * between the morphological closing of the input image and the image itself. This implementation 
 * utilizes CUDA for efficient execution on the GPU.
 *
 * This version applies closing by reconstruction instead of direct subtraction, preserving edge details. 
 * The closed image serves as a marker for grayscale reconstruction, where dilation progressively 
 * refines the output until convergence, ensuring accurate segmentation of porosity.
 *
...
void bottom_hat_reconstruction_on_device(dtype* hostImage, dtype* hostOutput, const int xsize,
                                         ...
                                         int kernel_zsize);
/**
 * @brief Computes a specialized bottom-hat transformation of a grayscale image on the CPU.
 *
 * The bottom-hat transformation enhances dark structures in an image by computing the difference 
 * between the morphological closing of the input image and the image itself. This implementation 
 * is performed entirely on the host CPU.
 *
 * This version applies closing by reconstruction, preserving edge details. The closed image serves 
 * as a marker for grayscale reconstruction, where dilation progressively refines the output until 
 * convergence, ensuring accurate segmentation of porosity.
 *
 * @tparam dtype Data type of the image (e.g., int, unsigned int, float).
 * @param hostImage Pointer to the input image stored on the host.
 * @param hostOutput Pointer to the output image stored on the host.
...
#endif  // BOTTOM_HAT_RECONSTRUCTION_H