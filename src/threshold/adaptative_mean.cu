                     ...
                     int nx, int ny, int nz, float weight)
{
  ...
  local_mean_kernel_3d<<<grid, block>>>(deviceImage, deviceOutput, weight, xsize, ysize, zsize,
                                                   nx, ny, nz);
...
template void adaptativeMeanThreshold3DGPU<float, float>(float* hostImage, float* hostOutput,
                                            int xsize, int ysize, int zsize, int flag_verbose,
                                            int nx, int ny, int nz, float weight);

template void adaptativeMeanThreshold3DGPU<int, float>(int* hostImage, float* hostOutput,
                                            int xsize, int ysize, int zsize, int flag_verbose,
                                            int nx, int ny, int nz, float weight);

template void adaptativeMeanThreshold3DGPU<unsigned int, float>(unsigned int* hostImage, float* hostOutput,
                                            int xsize, int ysize, int zsize, int flag_verbose,
                                            int nx, int ny, int nz, float weight);
                       ...
                       float gpuMemory, int ngpus, int nx, int ny, int nz)
{
  ...
  {
    //calls 2d variant
    local_mean_threshold(hostImage, hostOutput,weight,xsize,ysize,zsize,nx,ny,1);
    std::cout<<"2d variant\n";
...
template void adaptativeMeanThresholdChunked<float, float>(float* hostImage, float* hostOutput,
                                              int xsize, int ysize, int zsize, float weight, int type3d, int flag_verbose,
                                              float gpuMemory, int ngpus, int nx, int ny, int nz);

template void adaptativeMeanThresholdChunked<int, float>(int* hostImage, float* hostOutput,
                                            int xsize, int ysize, int zsize, float weight,int type3d,int flag_verbose,
                                            float gpuMemory, int ngpus, int nx, int ny, int nz);

template void adaptativeMeanThresholdChunked<unsigned int, float>(unsigned int* hostImage, float* hostOutput,
                                                     int xsize, int ysize, int zsize, float weight,int type3d,int flag_verbose,
                                                     float gpuMemory, int ngpus, int nx, int ny, int nz);
