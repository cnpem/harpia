...
template<typename dtype>
__global__ void niblack_kernel_3d(dtype* image, float* output,float weight,
...
template __global__ void niblack_kernel_2d<int>(int* image, float* output, float weight, int rows, int cols, int idz, int rows_kernel, int cols_kernel);
template __global__ void niblack_kernel_2d<float>(float* image, float* output, float weight, int rows, int cols, int idz, int rows_kernel, int cols_kernel);

template __global__ void niblack_kernel_3d<int>(int* image, float* output, float weight, int rows, int cols, int depth,int rows_kernel, int cols_kernel, int depth_kernel);
template __global__ void niblack_kernel_3d<float>(float* image, float* output, float weight, int rows, int cols, int depth,int rows_kernel, int cols_kernel, int depth_kernel);
...
void niblack_threshold(dtype* image, float* output, float weight, int rows, int cols, int depth, int rows_kernel, int cols_kernel, int depth_kernel)
{
    ...
    if (depth_kernel == 1)
    {
        ...
        {
            niblack_kernel_2d<<<gridSize, blockSize>>>(dev_image, dev_output, weight, rows, cols, idz, rows_kernel, cols_kernel);
    ...
    else
    {
        ...
        niblack_kernel_3d<<<gridSize, blockSize>>>(dev_image, dev_output, weight, rows, cols, depth, rows_kernel, cols_kernel, depth_kernel);
