...
__global__ void local_gaussian_kernel_2d(dtype* image, float* output,float* dev_kernel, float weight,
...
template<typename dtype>
__global__ void local_gaussian_kernel_3d(dtype* image, float* output,float* dev_kernel, float weight,
...
template __global__ void local_gaussian_kernel_2d<int>(int* image, float* output, float* dev_kernel, float weight,int idz, int rows, int cols, int slices, int rows_kernel, int cols_kernel);
template __global__ void local_gaussian_kernel_2d<float>(float* image, float* output, float* dev_kernel, float weight, int idz, int rows, int cols, int slices, int rows_kernel, int cols_kernel);

template __global__ void local_gaussian_kernel_3d<int>(int* image, float* output,float* dev_kernel, float weight,int rows, int cols, int depth,int rows_kernel, int cols_kernel, int depth_kernel);
template __global__ void local_gaussian_kernel_3d<float>(float* image, float* output,float* dev_kernel, float weight,int rows, int cols, int depth,int rows_kernel, int cols_kernel, int depth_kernel);


template<typename dtype>
void local_gaussian_threshold(dtype* image, float* output, int rows, int cols, int slices, float sigma, float weight, bool type)
{
    ...
    if (type == false)
    {
        ...
        {
            local_gaussian_kernel_2d<<<gridSize, blockSize>>>(dev_image, dev_output, dev_kernel, weight, k, rows, cols, slices, rows_kernel, cols_kernel);
        }
    ...
    else
    {
        ...
        local_gaussian_kernel_3d<<<gridSize, blockSize>>>(dev_image, dev_output,dev_kernel, weight, rows, cols, slices, rows_kernel, cols_kernel, depth_kernel);
