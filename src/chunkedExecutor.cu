...
void chunkedExecutorKernel(Func func, int ncopies, const float safetyMargin, dtype* hostImage,
  ...
  if (chunkSize == 0) {
    ...
    func(i_ref, o_ref, xsize, ysize, zsize, padding_bottom, padding_top, kernel, kernel_xsize,
         kernel_ysize, kernel_zsize, args...);
    if (verbose) {
      printf("\nFinished processing!\n");
    }
    return;
  ...
  padding_top = padding;
  if (verbose) {
    printf("Padding bottom:%d padding top:%d chunk:%d\n", padding_bottom, padding_top,
           chunkSize + padding);
  }
  if (verbose) {
    printf("iz:%d\n", iz);
  }
  func(i_ref, o_ref, xsize, ysize, chunkSize + padding, padding_bottom, padding_top, kernel,
  ...
  // Process the middle chunks
  padding_bottom = padding;
  int effectiveChunkSize = chunkSize + 2 * padding;
  for (iz = chunkSize; iz <= zsize - chunkSize; iz += chunkSize) {
    int remaining = zsize - iz - chunkSize;  // Check if this is the last chunk
    if (remaining <= 0) {
      effectiveChunkSize = chunkSize + padding;  // Only add padding at the start for the last chunk
      padding_top = 0;
    }
    if (verbose) {
      printf("Padding bottom:%d padding top:%d chunk:%d\n", padding_bottom, padding_top,
    ...
    func(i_ref - padding * sliceSize, o_ref - padding * sliceSize, xsize, ysize, effectiveChunkSize,
         padding_bottom, padding_top, kernel, kernel_xsize, kernel_ysize, kernel_zsize, args...);
  ...
  if (remaining > 0) {
    if (verbose) {
      printf("\nProcessing remaining last chunk with padding at the start, remaining:%d\n",
             remaining);
    }
    padding_top = 0;
    int effectiveRemaining =
        remaining + padding;  // Add padding only at the start for the last chunk
    if (verbose) {
      printf("Padding bottom:%d padding top:%d chunk:%d\n", padding_bottom, padding_top,
             effectiveRemaining);
    }
    ...
    func(i_ref - padding * sliceSize, o_ref - padding * sliceSize, xsize, ysize, effectiveRemaining,
         padding_bottom, padding_top, kernel, kernel_xsize, kernel_ysize, kernel_zsize, args...);
  }
...
// // Wrapper function
// template <typename Func, typename dtype, typename... Args>
// void chunkedExecutorMask(Func func, int ncopies, const float safetyMargin, dtype* hostImage,
//                      dtype* hostOutput, const int xsize, const int ysize, const int zsize, const int kernel_zsize,
//                      const int verbose, Args... args) {
