...
void fraction(int* image, int* output, int xsize, int ysize, int zsize)
{
    ...
    fraction_counter<<<gridDim,blockDim>>>(deviceImage,deviceOutput,*deviceAcumulator,xsize,ysize,zsize);

    labels_fraction<<<gridDim,blockDim>>>(deviceImage,deviceOutput,*deviceAcumulator,xsize,ysize,zsize);
