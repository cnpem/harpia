...
void chunkedExecutorPixelFeatures(Func func, int ncopies, int nFeatures, const float safetyMargin, int ngpus, 
  ...
  int deviceCount = 0;
  int selectedDevice;
  for (; z_offset <= zsize - chunkSize; z_offset += chunkSize) {
    selectedDevice = deviceCount % ngpus;
    CHECK(cudaSetDevice(selectedDevice));
    cudaDeviceSynchronize();
    if (verbose) {
      printf("Processing chunk: z_offset=%d, chunkSize=%d, device=%d\n", z_offset, chunkSize, selectedDevice);
    }
    if (verbose) {
  ...
  if (remaining > 0) {
    func(i_ref, o_ref, xsize, ysize, remaining, z_offset, total_volume, verbose, args...);
  }
  if (verbose) {
