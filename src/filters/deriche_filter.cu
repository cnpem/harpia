...
void get_deriche_vertical_kernel_3d(float** kernel, float alpha, float beta, int xsize, int ysize, int zsize) {
    float c = ((1 - exp(-alpha)) * (1 - exp(-alpha))) / exp(-alpha);
    float k = (((1 - exp(-alpha)) * (1 - exp(-alpha))) * (alpha * alpha)) / (1 - 2 * alpha * exp(-alpha) - exp(-2 * alpha));
...
//done
__global__ void deriche_non_maximum_supression_2d(float* magnitude, uint8_t* direction, int xsize, int ysize, int idz)
{
...
__global__ void deriche_thresholding_2d(float* image, float low, float high, int xsize, int ysize, int idz)
{
                     ...
                     float alpha, float low_threshold, float high_threshold)
{
    ...
    */
    ...
    cudaMemcpy(dev_horizontal_kernel, horizontal_kernel, kx * ky * sizeof(float), cudaMemcpyHostToDevice);
   ...
   for (int k = 0; k < zsize; k++)
   {
        deriche_non_maximum_supression_2d<<<gridSize,blockSize>>>(dev_magnitude, dev_direction, xsize, ysize, k);
   }
   ...
   */
    ...
    for (int k = 0; k < zsize; k++)
    {
        ...
        deriche_thresholding_2d<<<gridSize, blockSize>>>(dev_magnitude,low_threshold, high_threshold, xsize, ysize, k);
