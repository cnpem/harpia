...
void test_smooth_binary_on_device(const std::string& filename, const int xsize, const int ysize,
  ...
  int ncopies = 2;
  int operations = 4;
  chunkedExecutorKernel(smooth_binary_on_device<int>, ncopies, memoryOccupancy, ngpus, operations, host_A,
  ...
  if (flag_check) {
    ...
    smooth_binary_on_host(host_A, host_ref, xsize, ysize, zsize, kernel, kernel_xsize, kernel_ysize,
                          kernel_zsize);
...
void test_smooth_binary_on_host(const std::string& filename, const int xsize, const int ysize,
  ...
  if (flag_check) {
    ...
    morphology_3D_openCV(opencv_tmp, opencv_ref, xsize, ysize, zsize, kernel_xsize, kernel_ysize,
                         DILATION);
    morphology_3D_openCV(opencv_ref, opencv_tmp, xsize, ysize, zsize, kernel_xsize, kernel_ysize,
                         DILATION);
    morphology_3D_openCV(opencv_tmp, opencv_ref, xsize, ysize, zsize, kernel_xsize, kernel_ysize,
                         EROSION);
    if (flag_show)
